...
export default function App() {
  ...
  const [matches, setMatches] = useState(() => [
    {
      ...
      // playerStats: { [userId]: { goals, assists } }
      // highlights: [{ userId, label }]
      // admin: { status, plannedDurationMin, startedAt, endedAt, delayMin, notes }
      // discipline: { [userId]: { yellow, red, events:[...] } }
    },
  ]);
  ...
  // VIEW CONTROL
  // =========================================================
  const [view, setView] = useState("login"); // ✅ começa no login por padrão
  ...
  // ✅ abre o modal com base no matchId
  function onManageStats(matchId) {
    setAdminMatchId(matchId);
  }

  // ✅ salva ADMIN COMPLETO no match (state local por enquanto)
  ...
  // ✅ controle do menu criar via role do backend
  ...
  // ✅ Se estiver em login/register, não faz sentido mostrar menu completo
  ...
  return (
    ...
    >
      ...
      {/* ✅ MODAL ADMIN — aparece quando clicar "Gerenciar Súmula" */}
      {adminMatch ? (
        <MatchAdminModal
          match={adminMatch}
          courts={courts}
          user={user}
          users={users} // ✅ usa a mesma fonte agora
